package com.fz.admin.framework.datapermission.rule;
...
public class DataPermissionRuleFactoryImpl implements DataPermissionRuleFactory {
    ...
    public List<DataPermissionRule> getDataPermissionRule(String mappedStatementId) {
        ...
        // 4. 已配置，只选择部分规则
        if (!ObjectUtils.isEmpty(dataPermission.includeRules())) {
            return rules.stream().filter(rule -> Arrays.asList(dataPermission.includeRules()).contains(rule.getClass()))
                    .collect(Collectors.toList()); // 一般规则不会太多，所以不采用 HashSet 查询
        }
        // 5. 已配置，只排除部分规则
        if (!ObjectUtils.isEmpty(dataPermission.excludeRules())) {
            return rules.stream().filter(rule -> !Arrays.asList(dataPermission.excludeRules()).contains(rule.getClass()))
                    .collect(Collectors.toList()); // 一般规则不会太多，所以不采用 HashSet 查询
        }
        // 6. 已配置，全部规则
        return rules;
