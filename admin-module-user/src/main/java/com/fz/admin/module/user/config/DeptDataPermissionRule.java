...
import com.fz.admin.framework.security.util.SecurityContextUtils;
...
public class DeptDataPermissionRule implements DataPermissionRule {
     ...
     */
    private final Set<String> tablesNames = new HashSet<>();

    @PostConstruct
    private void init() {

        // 部门列
        deptColumns.putAll(Map.of(
                "sys_user", "dept_id",
                "sys_dept", "id"
        ));

        userColumns.put("sys_user", "id");

        tablesNames.addAll(List.of("sys_user", "sys_dept"));
    ...
    public Expression getExpression(String tableName, Alias tableAlias) {
        ...
        // 获得数据权限
        RoleDataScope roleDataScope = (RoleDataScope) loginUser.getContext(CONTEXT_KEY);
        // 从上下文中拿不到，则调用逻辑进行获取
        if (roleDataScope == null) {
            roleDataScope = roleService.getDataScopeByUserId(loginUser.getId());
            if (roleDataScope == null) {
                log.error("[getExpression][LoginUser({}) 获取数据权限为 null]", Json.toJsonString(loginUser));
                throw new NullPointerException(String.format("LoginUser(%d) Table(%s/%s) 未返回数据权限",
                        loginUser.getId(), tableName, tableAlias.getName()));
            }
            // 添加到上下文中，避免重复计算
            loginUser.setContext(CONTEXT_KEY, roleDataScope);
        }

        // 情况一，如果是 ALL 可查看全部，则无需拼接条件
        if (roleDataScope.getAll()) {
            return null;
        }

        // 情况二，即不能查看部门，又不能查看自己，则说明 100% 无权限
        if (ObjectUtils.isEmpty(roleDataScope.getDeptIds())
                && Boolean.FALSE.equals(roleDataScope.getSelf())) {
            return new EqualsTo(null, null); // WHERE null = null，可以保证返回的数据为空
        }

        // 情况三，拼接 Dept 和 User 的条件，最后组合
        Expression deptExpression = buildDeptExpression(tableName, tableAlias, roleDataScope.getDeptIds());
        Expression userExpression = buildUserExpression(tableName, tableAlias, roleDataScope.getSelf(), loginUser.getId());
        if (deptExpression == null && userExpression == null) {
            // TODO 芋艿：获得不到条件的时候，暂时不抛出异常，而是不返回数据
            log.warn("[getExpression][LoginUser({}) Table({}/{}) DeptDataPermission({}) 构建的条件为空]",
                    Json.toJsonString(loginUser), tableName, tableAlias, Json.toJsonString(roleDataScope));
...
//                    loginUser.getId(), tableName, tableAlias.getName()));
    ...
    private Expression buildDeptExpression(String tableName, Alias tableAlias, Set<Long> deptIds) {
        ...
        return new InExpression(MyBatisUtils.buildColumn(tableName, tableAlias, columnName),
                new ExpressionList(CollectionConverter.convertList(deptIds, LongValue::new)));
    }
